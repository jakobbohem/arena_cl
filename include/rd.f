...
* Fullsource for module RD from package SLATEC.
* Retrieved from TIBER on Mon Apr 29 04:59:46 1996.
* ======================================================================
*DECK RD
...
C         CALL XERMSG ('SLATEC', 'RD',
C     *      'MIN(X,Y).LT.0 WHERE X = ' // XERN3 // ' AND Y = ' //
C     *      XERN4, 1, 1)
         write (*,*) 'slatec, rd: MIN(X,Y).LT.0 WHERE X = ' // XERN3
     *   // ' AND Y = ' // XERN4
         RETURN
      ENDIF
C
...
C         CALL XERMSG ('SLATEC', 'RD',
C     *      'MAX(X,Y,Z).GT.UPLIM WHERE X = ' // XERN3 // ' Y = ' //
C     *      XERN4 // ' Z = ' // XERN5 // ' AND UPLIM = ' // XERN6,
C     *      3, 1)
         write (*,*) 'slatec, rd: MAX(X,Y,Z).GT.UPLIM WHERE X = ' //
     *   XERN3 // ' Y = ' // XERN4 // ' Z = ' // XERN5 // ' AND
     * UPLIM = ' // XERN6
         RETURN
      ENDIF
C
...
C         CALL XERMSG ('SLATEC', 'RD',
C     *      'MIN(X+Y,Z).LT.LOLIM WHERE X = ' // XERN3 // ' Y = ' //
C     *      XERN4 // ' Z = ' // XERN5 // ' AND LOLIM = ' // XERN6,
C     *      2, 1)
         write (*,*) 'slatec, rd: MIN(X+Y,Z).LT.LOLIM WHERE X = ' //
     *   XERN3 // ' Y = ' //
